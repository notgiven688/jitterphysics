...
namespace Jitter2D
{
    ...
    public class World
    {
        ...
        private JVector gravity = new JVector(0, -9.81f);
        ...
        private void IntegrateForces()
        {
            foreach (RigidBody body in rigidBodies)
            {
                if (!body.isStatic && body.IsActive)
                {
                    ...
                    if (!(body.isParticle))
                    {
                        body.angularVelocity += body.torque * body.invInertia * timestep;
                    }
            ...
            foreach (CollisionIsland island in islands)
            {
                ...
                else
                {
                    foreach (RigidBody body in island.bodies)
                    {
                        ...
                        if (body.AllowDeactivation && (body.angularVelocity < inactiveAngularThresholdSq &&
